...
### このForkで追加した機能: Web管理画面

初回セットアップ後、USBメモリを再度使わなくても、ブラウザから `http://<Cube J1のIPアドレス>:8080/` にアクセスすることで以下を変更できます。

- Wi-Fi の SSID / パスワード
- `config.json` の全項目（Bルート認証情報、MQTT接続先、ポーリング間隔など）
- ブリッジの動作ログ表示
- 本体の再起動

保存すると自動的に Wi-Fi の再接続、またはブリッジの再起動が行われ、設定が反映されます。
