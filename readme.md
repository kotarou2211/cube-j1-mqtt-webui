...
2. `production_tool/config.json` を編集して、認証情報やネットワークの接続先を設定する（詳細は[config.json の設定](#configjson-の設定)を参照）。空欄のままでも次のステップには進めます。
3. `production_tool/wpa_supplicant.conf` は**自宅 Wi-Fi 情報が分からない場合、空欄のままで構いません**。あらかじめ分かっているなら SSID とパスワードを記入しておいても構いません（詳細は[スマホだけでWi-Fiを設定する](#スマホだけでwi-fiを設定する)を参照）。
...
6. 自動的にスクリプトが実行されます。セットアップが完了すると、**本体の LED が白色に 10 回点滅**します。Wi-Fi 接続情報が正しく設定されていて接続に成功した場合は、LED が緑色に点灯します（Wi-Fi 未設定の場合は緑色になりません。次項を参照）。
...
`wpa_supplicant.conf` を空欄のままセットアップした場合、Cube J1 は自宅 Wi-Fi に接続できていない状態で起動します。この場合でも、Cube J1 が標準で発信している Wi-Fi Direct アクセスポイント（**`CubeJ-XXXXXX`**、`XXXXXX` は本体ごとに異なる MAC アドレス下 6 桁）経由で Web 管理画面にアクセスし、自宅 Wi-Fi を設定できます。

1. スマホ等の Wi-Fi 設定から **`CubeJ-XXXXXX`** に接続する（パスフレーズ: `12345678`）。
2. ブラウザで **`http://192.168.100.1:8080/`** を開く。
3. 「Wi-Fi 設定」欄に自宅の SSID とパスワードを入力し、保存する。
4. 保存と同時に Cube J1 が自宅 Wi-Fi への接続を試みます。成功すると本体 LED が緑色に点灯します。
アクセス後は管理画面から設定が可能です。
なお、SSIDは2.4Ghzのものしか接続できない為注意をしてください。
