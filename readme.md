...
以降は自動的に MQTT ブリッジおよび [Web管理画面](#web管理画面) が起動し、スマートメーターからのデータ取得と Home Assistant への送信が開始されます。
...
初回セットアップ後は、ブラウザで **`http://<Cube J1 の IP アドレス>:8080/`** にアクセスするだけで、以下の設定変更が行えます。USB メモリの抜き差しは不要です。
